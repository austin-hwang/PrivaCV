...
- **Review Text** — shows the exact ATS-friendly plain-text version before
  copying it for application forms, recruiter portals, and quick parsing checks.
...
Click **Review Text** to inspect a clean text version before copying. It uses
the same section order as the preview, keeps standard headings, and formats
bullets with simple hyphens so the content is easy to paste into job
applications.
