...
test("imports a pasted resume locally and keeps confirmation deliberate without repetitive clicks", async ({
  page,
}) => {
  await page.goto("/");
  await page.evaluate(() => localStorage.clear());
...
test("automatically checkpoints the current draft before a pasted import replaces it", async ({
  page,
}) => {
  await page.goto("/");
  await page.evaluate(() => localStorage.clear());
    ...
    .poll(() =>
      page.evaluate(() => {
        ...
        const checkpoint = history.find((item: { id?: string }) =>
          item.id?.startsWith("autosave-slot-"),
        );
        return {
...
test("keeps an unfinished import review after a browser refresh", async ({ page }) => {
  ...
  await expect
    .poll(async () => page.evaluate(() => localStorage.getItem("resume-editor-data-v2")))
    .toContain("Ada Lovelace");
  const storedReview = await page.evaluate(() =>
    localStorage.getItem("resume-editor-import-review-v1"),
  );
...
test("lets someone finish review of a clean imported draft without repetitive clicks", async ({
  page,
}) => {
  await page.goto("/");
  await page.evaluate(() => localStorage.clear());
...
test("imports common alternate section headings without losing resume content", async ({
  page,
}) => {
  await page.goto("/");
  await page.evaluate(() => localStorage.clear());
  ...
  await expect(summaryEditor(page)).toHaveText(
    "Platform engineer building dependable developer tools.",
  );
...
test("imports concise overview and skills headings without losing their content", async ({
  page,
}) => {
  await page.goto("/");
  await page.evaluate(() => localStorage.clear());
...
test("keeps an employer-first dated PDF header editable as the right role and employer", async ({
  page,
}) => {
  await page.goto("/");
  await page.evaluate(() => localStorage.clear());
...
test("requires review of imported specialty-section entries", async ({ page }) => {
  ...
  await expect(page.getByLabel("License / certification", { exact: true })).toHaveValue(
  ...
  for (let step = 0; step < 8; step += 1) {
    if (
      await tour
        .getByText("Certifications entry 1")
        .isVisible()
        .catch(() => false)
    )
      break;
...
test("highlights grouped Skills during the import review tour", async ({ page }) => {
  ...
  await importDialog
    .getByLabel("Resume text")
    .fill(
      ["Ada Lovelace", "ada@example.com", "", "Skills", "TypeScript, SQL, systems design"].join(
        "\n",
      ),
    );
  await importDialog.getByRole("button", { name: /^import text$/i }).click();
...
test("parses company-first imported experience entries and keeps the swap control available", async ({
  page,
}) => {
  await page.goto("/");
  await page.evaluate(() => localStorage.clear());
...
test("asks users to review every imported experience entry", async ({ page }) => {
  ...
  await expect(page.getByLabel("Job Title", { exact: true }).nth(1)).toHaveValue(
    "Software Engineer",
  );
...
test("keeps compact education entries separate when dates are on their own lines", async ({
  page,
}) => {
  await page.goto("/");
  await page.evaluate(() => localStorage.clear());
  ...
  await expect(page.getByLabel("Degree", { exact: true }).nth(0)).toHaveValue(
    "Master of Science in Computer Science",
  );
  await expect(page.getByLabel("Degree", { exact: true }).nth(1)).toHaveValue(
    "Bachelor of Science in Mathematics",
  );
});
...
test("calls out a specialty heading when import cannot reconstruct its entries", async ({
  page,
}) => {
  await page.goto("/");
  await page.evaluate(() => localStorage.clear());
  ...
  for (let step = 0; step < 6; step += 1) {
    if (
      await tour
        .getByText("Possible skipped section")
        .isVisible()
        .catch(() => false)
    )
      break;
...
test("switches between focused editor and preview views on a narrow screen", async ({ page }) => {
  ...
  await exportPdf(page);
  await page
    .getByRole("dialog", { name: /review before exporting/i })
    .getByRole("button", { name: /fix contact/i })
    .click();
  await expect(editorPane).toBeVisible();
