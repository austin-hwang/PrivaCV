...
test("publishes crawlable tracker and Sankey pages while keeping the workspace private", async ({ page, request }) => {
  ...
  await expect(page.getByRole("heading", { level: 1 })).toHaveText("A free, private job application tracker");
  await expect(page.locator('link[rel="canonical"]')).toHaveAttribute("href", "https://privacv.app/job-application-tracker");
  await expect(page.locator('meta[name="robots"]')).toHaveAttribute("content", "index, follow");
  await expect(page.locator('meta[property="og:image"]')).toHaveAttribute("content", "https://privacv.app/social/job-application-tracker");
  await expect(page.getByRole("link", { name: "Track your applications" })).toHaveAttribute("href", "/applications");
  expect((await page.locator('script[type="application/ld+json"]').allTextContents()).some((value) => value.includes('"@type":"WebApplication"'))).toBe(true);
  ...
  await expect(page.locator('meta[property="og:title"]')).toHaveAttribute("content", "Private Job Application Tracker | PrivaCV");
  await expect(page.locator('meta[property="og:image"]')).toHaveAttribute("content", "https://privacv.app/social/job-application-tracker");
