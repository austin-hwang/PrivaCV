...
import { ArrowDown, ArrowUp, ChevronRight, Eye, EyeOff, GripVertical, MoreHorizontal, Plus, Trash2 } from "lucide-react";
...
import type { LocalAIInlineTarget, useResumeWorkspaceUI } from "@/features/resume/hooks/use-resume-workspace-ui";
import {
...
export function ResumeSectionList({
  ...
  const {
    addBuiltinSection, addCustomSection, addEntry, moveEntry, moveSection, removeBuiltinSection,
    removeCustomSection, removeEntry, reorderEntry, reorderSection, state,
    swapExperienceTitleAndCompany, toggleSectionHidden, updateEntry, updateSectionFormat,
    updateSectionTagGroups, updateSectionText, updateSectionTitle,
  } = editor;
  const {
    activeTarget, collapsedGroups, draggedSection, dropTargetSection, localAIEnabled,
    localAIInlineTarget, setActiveTarget, setCollapsedGroups, setDraggedSection, setDropTargetSection,
  } = ui;
  const removedBuiltinSections = SECTION_KEYS.filter((section) => !state.sectionOrder.includes(section));
  const toggleGroup = (groupId: string) => setCollapsedGroups((previous) => {
  ...
  const startSectionDrag = (event: DragEvent<HTMLElement>, section: string, title: string) => {
    ...
    Object.assign(dragImage.style, {
      position: "fixed", top: "-1000px", left: "-1000px", display: "flex", alignItems: "center",
      border: "1px solid rgb(148 163 184)", borderRadius: "6px", background: "white",
      boxShadow: "0 8px 20px rgb(15 23 42 / 22%)", padding: "8px 12px", color: "rgb(15 23 42)",
  ...
  return (
    <section id="section-order-controls" tabIndex={-1} className="scroll-mt-44 space-y-3 lg:scroll-mt-16">
      ...
      {state.sectionOrder.map((section, sectionIndex) => {
        ...
        return (
          <div
            ...
            onDragEnter={(event) => {
              if (draggedSection && (event.dataTransfer.types.includes("application/x-resume-section") || event.dataTransfer.types.includes("text/plain"))) setDropTargetSection(section);
            }}
            onDragOver={(event) => {
          ...
          >
            <FieldGroup
              ...
              header={
                <div className="flex min-w-0 items-center gap-1.5">
                  ...
                  <span className="flex size-5 shrink-0 items-center justify-center rounded-full bg-muted text-[10px] font-semibold tabular-nums text-muted-foreground">{sectionIndex + 1}</span>
                  <Input
                  ...
                  />
                  {sectionCount > 0 ? <span className="editor-pane-wide shrink-0 tabular-nums text-xs text-muted-foreground" title={`${sectionCount} ${sectionCount === 1 ? "item" : "items"}`}>({sectionCount})</span> : null}
                  {sectionFormat === "tag-groups" ? (
                    <Button
                      ...
                      onClick={() => {
                        ...
                        expandGroup(section);
                        updateSectionTagGroups(section, [...tagGroups, { id: groupId, label: "", tags: [] }]);
                    ...
                    <Button id={`add-${section}-entry`} type="button" variant="ghost" size="sm" className="h-8 shrink-0 px-2" onClick={() => { expandGroup(section); addEntry(section); }}>
                      <Plus /> <span className="editor-pane-wide">Add</span>
                    </Button>
                  ) : null}
                  <Button
                  ...
                  <Menu>
                    <MenuTrigger><Button type="button" variant="ghost" size="icon" className="size-8 shrink-0" aria-label={`More actions for ${sectionDisplayTitle}`}><MoreHorizontal /></Button></MenuTrigger>
                    <MenuContent>
                      <MenuItem disabled={sectionIndex === 0} onSelect={() => moveSection(section, -1)}><ArrowUp /> Move section up</MenuItem>
                      <MenuItem disabled={sectionIndex === state.sectionOrder.length - 1} onSelect={() => moveSection(section, 1)}><ArrowDown /> Move section down</MenuItem>
                      <MenuSeparator />
                      <MenuItem destructive onSelect={() => (customSection ? removeCustomSection(section) : removeBuiltinSection(section))}><Trash2 /> Remove section</MenuItem>
                    </MenuContent>
                  </Menu>
                  <button
                    type="button"
            ...
            >
              <div className="grid gap-1.5 text-xs font-medium text-muted-foreground">
                ...
                <div role="group" aria-labelledby={`section-format-${section}`} className="flex w-full overflow-x-auto rounded-md border border-input bg-background shadow-sm">
                  {([
      ...
      <div className="rounded-lg border border-dashed bg-muted/20 p-3">
        ...
        <p className="mt-1 text-xs leading-snug text-muted-foreground">Choose a familiar heading or make your own.</p>
        <div className="mt-3 flex flex-wrap gap-2">
          {removedBuiltinSections.map((section) => (
            <Button key={section} type="button" variant="outline" size="sm" onClick={() => { addBuiltinSection(section); focusEditorTarget(`section-title-${section}`); }}>
              <Plus /> {SECTION_LABELS[section]}
            </Button>
          ))}
          {CUSTOM_SECTION_PRESETS.map((title) => (
            <Button key={title} type="button" variant="outline" size="sm" onClick={() => { const id = addCustomSection(title, CUSTOM_SECTION_PRESET_FORMATS[title]); focusEditorTarget(`section-title-${id}`); }}>
              <Plus /> {title}
            </Button>
          ))}
          <Button id="add-custom-section" type="button" variant="secondary" size="sm" onClick={() => { const id = addCustomSection(); focusEditorTarget(`section-title-${id}`); }}>
