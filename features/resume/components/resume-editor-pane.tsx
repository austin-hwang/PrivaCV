...
import { AlertCircle, ArrowRight, Check, ChevronsDownUp, ChevronsUpDown, Download, RotateCcw, Search, Undo2 } from "lucide-react";
...
export function ResumeEditorPane({
  ...
  const {
    completeImportReview, dismissRecoveryPoint, externalDraft, historyBackupInputRef, importFileInputRef,
    importReview, importReviewStatus, isImporting, jsonInputRef, keepCurrentDraft, loadSample,
    recoveryPoint, restoreRecoveryPoint, saveJson, setTextImportOpen, state, storageIssue,
    updateField, useExternalDraft,
  } = editor;
  const {
    activeTarget, collapsedGroups, editorCollapsed, localAIEnabled, localAIInlineTarget,
    mobileWorkspaceView, workspaceHasStarted, setActiveTarget, setBlankTemplatePreview,
    setCollapsedGroups, setDestructiveAction,
  } = ui;
    ...
    >
        ...
        {externalDraft ? (
          <Alert className="mb-6 border-brand/40 bg-brand/10">
            ...
            <AlertDescription className="grid gap-3">
              ...
              {externalDraftChanges.length ? <ChangeSummaryGrid changes={externalDraftChanges} beforeLabel="This tab" afterLabel="Saved tab" /> : null}
              <span className="flex shrink-0 flex-wrap gap-2">
                <Button type="button" variant="outline" size="sm" className="border-brand/40 bg-background" onClick={useExternalDraft}>Use saved draft</Button>
                <Button type="button" variant="outline" size="sm" className="border-brand/40 bg-background" onClick={keepCurrentDraft}>Keep this draft</Button>
              </span>
            </AlertDescription>
          </Alert>
        ) : null}
          ...
          <Card className="mb-6 border-brand/30 bg-brand/10">
            <CardHeader data-recovery-header className="editor-pane-row flex gap-3 space-y-0">
              <div>
                ...
                <CardDescription>{recoveryPoint.label}. Stays until you restore or dismiss it.</CardDescription>
              </div>
              <div className="editor-pane-actions flex shrink-0 gap-2">
                <Button type="button" variant="outline" size="sm" onClick={restoreRecoveryPoint}><Undo2 /> Restore previous</Button>
                <Button type="button" variant="ghost" size="sm" onClick={dismissRecoveryPoint}>Dismiss</Button>
              </div>
            </CardHeader>
          </Card>
        ) : null}
        ...
        {importReview ? (
          <Card id="import-review-panel" className="mb-6 border-warning/30 bg-warning/10">
            <CardHeader className="space-y-2">
              <div className="editor-pane-row flex gap-3">
                <div>
                  ...
                  <CardDescription>Imported from {importReview.fileName}. Check each field and confirm.</CardDescription>
                </div>
            ...
            <CardContent className="flex flex-wrap items-center gap-2">
              <Button type="button" size="sm" onClick={startImportTour}><ArrowRight /> {(importReviewStatus?.reviewedCount ?? 0) > 0 ? "Continue walkthrough" : "Start walkthrough"}</Button>
              <Button type="button" variant="outline" size="sm" onClick={() => completeImportReview(true)}><Check /> Finish review</Button>
            </CardContent>
          </Card>
        ) : null}
      </div>
      ...
      {workspaceHasStarted ? (
        <div className="-mt-2 mb-1 flex justify-end gap-1">
          <Button type="button" variant="ghost" size="sm" className="h-7 gap-1.5 px-2 text-xs text-muted-foreground" onClick={toggleNavigator}><Search /> Navigate</Button>
          <Button type="button" variant="ghost" size="sm" className="h-7 gap-1.5 px-2 text-xs text-muted-foreground" onClick={() => setCollapsedGroups(allCollapsed ? new Set() : new Set(editorGroupIds))}>
            {allCollapsed ? <ChevronsUpDown /> : <ChevronsDownUp />} {allCollapsed ? "Expand all" : "Collapse all"}
          </Button>
          <Button type="button" variant="ghost" size="sm" className="h-7 gap-1.5 px-2 text-xs text-destructive hover:bg-destructive/10 hover:text-destructive" onClick={() => setDestructiveAction("clear")}><RotateCcw /> Clear</Button>
        </div>
      ) : null}
