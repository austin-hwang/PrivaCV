...
describe("resume workspace builders", () => {
  it("builds stable section and field navigation from resume state", () => {
    ...
    expect(buildSectionNavItems(state, true)).toEqual(
    ...
    expect(buildNavigatorItems(state, true)).toEqual(
      expect.arrayContaining([
  ...
  it("keeps import and check actions connected to their coordinator callbacks", () => {
    ...
    const skippedCoverage = buildImportCoverage(state, sourceText).filter(
      (item) => item.sourceDetected && !item.detected,
    );
    const importSteps = buildImportTourSteps({
      importReview,
      skippedCoverage,
      onToggleItem: toggleItem,
      onFocusTarget: focusImportTarget,
    });
