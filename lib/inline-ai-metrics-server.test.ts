...
describe("anonymous inline AI metrics", () => {
  it.each(["inline_ai_used", "inline_ai_accepted"])(
    "records only the %s milestone",
    async (event) => {
  ...
  it("rejects unknown milestones and cross-site submissions", async () => {
    ...
    expect((await handleInlineAIMetric(metricRequest({ event: "prompt_text" }), env))?.status).toBe(
      400,
    );
    expect(
      (
        await handleInlineAIMetric(
          metricRequest(
