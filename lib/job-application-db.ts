...
function requestResult<T>(request: IDBRequest<T>) {
  return new Promise<T>((resolve, reject) => {
    ...
    request.onerror = () => reject(request.error ?? new Error("The browser database request failed."));
  });
}
...
function transactionComplete(transaction: IDBTransaction) {
  return new Promise<void>((resolve, reject) => {
    ...
    transaction.onerror = () => reject(transaction.error ?? new Error("The browser database transaction failed."));
    transaction.onabort = () => reject(transaction.error ?? new Error("The browser database transaction was cancelled."));
  });
}
...
export async function loadJobPipelineData(): Promise<JobPipelineData> {
  ...
  const applicationsRequest = transaction.objectStore(APPLICATIONS_STORE).getAll() as IDBRequest<JobApplication[]>;
  const eventsRequest = transaction.objectStore(EVENTS_STORE).getAll() as IDBRequest<ApplicationEvent[]>;
  const jobSnapshotsRequest = transaction.objectStore(JOB_SNAPSHOTS_STORE).getAll() as IDBRequest<JobSnapshot[]>;
  const resumeSnapshotsRequest = transaction.objectStore(RESUME_SNAPSHOTS_STORE).getAll() as IDBRequest<ResumeSnapshot[]>;
...
export async function updateStoredJobApplication(applicationId: string, update: JobApplicationUpdate) {
  const database = await openJobPipelineDatabase();
  const existing = await requestResult(
    database.transaction(APPLICATIONS_STORE, "readonly").objectStore(APPLICATIONS_STORE).get(applicationId) as IDBRequest<JobApplication | undefined>,
  );
...
export async function deleteStoredJobApplication(applicationId: string) {
  ...
  const eventKeys = await requestResult(
    database.transaction(EVENTS_STORE, "readonly").objectStore(EVENTS_STORE).index("applicationId").getAllKeys(applicationId),
  );
  const resumeKeys = await requestResult(
    database.transaction(RESUME_SNAPSHOTS_STORE, "readonly").objectStore(RESUME_SNAPSHOTS_STORE).index("applicationId").getAllKeys(applicationId),
  );
  const transaction = database.transaction(
    [APPLICATIONS_STORE, EVENTS_STORE, JOB_SNAPSHOTS_STORE, RESUME_SNAPSHOTS_STORE],
    "readwrite",
...
export function parseJobPipelineBackup(value: string): JobPipelineBackup {
  ...
  if (!Array.isArray(parsed.applications) || !Array.isArray(parsed.events) || !Array.isArray(parsed.jobSnapshots) || !Array.isArray(parsed.resumeSnapshots)) {
  ...
  const applications = parsed.applications.flatMap((item): JobApplication[] => {
    ...
    const optionalString = (field: string) => typeof item[field] === "string" ? item[field] as string : "";
    return [{
      id: item.id,
  ...
  const events = parsed.events.filter((item): item is ApplicationEvent => (
    isRecord(item)
    && typeof item.id === "string"
  ...
  const jobSnapshots = parsed.jobSnapshots.filter((item): item is JobSnapshot => (
    isRecord(item)
    && typeof item.applicationId === "string"
  ...
  const resumeSnapshots = parsed.resumeSnapshots.filter((item): item is ResumeSnapshot => (
    isRecord(item)
    && typeof item.id === "string"
    && typeof item.applicationId === "string"
...
export async function restoreJobPipelineBackup(backup: JobPipelineBackup) {
  ...
  backup.resumeSnapshots.forEach((item) => transaction.objectStore(RESUME_SNAPSHOTS_STORE).put(item));
  await transactionComplete(transaction);
}
