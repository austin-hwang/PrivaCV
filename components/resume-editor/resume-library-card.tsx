...
function ResumeThumbnail({ item }: { item: ResumeLibraryItem }) {
  return (
    <div aria-hidden="true" className="relative h-[232px] overflow-hidden rounded-md border bg-muted/50 shadow-inner">
      <div
...
export function ResumeLibraryCard({
  ...
  const visibleItems = useMemo(() => {
    ...
    return items.filter((item) => [item.label, item.state.name, item.state.title]
      .some((value) => value.toLocaleLowerCase().includes(clean)));
  }, [items, query]);
    ...
    >
      <DialogContent className="flex h-[min(900px,calc(100dvh-2rem))] max-w-6xl flex-col gap-0 overflow-hidden p-0">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-b bg-muted/20 px-5 py-3">
          <p className="text-sm text-muted-foreground">{items.length} {items.length === 1 ? "resume" : "resumes"} saved in this browser</p>
          <Button type="button" size="sm" onClick={() => { onCreate(); onOpenChange(false); }}>
            <Plus /> New resume
          </Button>
        </div>
        <div className="min-h-0 flex-1 overflow-y-auto p-5">
          <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
            <div className="relative min-w-0 flex-1 sm:max-w-sm">
              ...
              />
              {query ? (
                <Button type="button" variant="ghost" size="icon" className="absolute right-0 top-0 size-9" aria-label="Clear resume search" onClick={() => setQuery("")}>
                  <X />
                </Button>
              ) : null}
            </div>
          ...
          {visibleItems.length ? (
                ...
                return (
                  <li key={item.id} className="rounded-lg border bg-card p-3 shadow-sm">
                    ...
                    <div className="mt-3 flex items-start gap-2">
                      <FileStack className="mt-0.5 size-4 shrink-0 text-muted-foreground" aria-hidden="true" />
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          {active ? <Badge variant="outline" className="h-5 shrink-0 px-1.5 text-[10px]">Current</Badge> : null}
                        </div>
                        <p className="mt-0.5 text-xs text-muted-foreground">Edited {formatCheckpointTime(item.updatedAt)} · {versionHeadline(item.state)}</p>
                      </div>
                    </div>
                    {deletingId === item.id ? (
                      <div className="mt-3 rounded-md border border-destructive/30 bg-destructive/5 p-2.5">
                        <p className="text-xs text-foreground">Delete this resume and its checkpoints?</p>
                        <div className="mt-2 flex gap-2">
                          <Button type="button" variant="outline" size="sm" onClick={() => setDeletingId(null)}>Cancel</Button>
                          <Button type="button" variant="destructive" size="sm" onClick={() => { onDelete(item.id); setDeletingId(null); }}>
                            Delete resume
                          </Button>
                        </div>
                      </div>
                      ...
                      <div className="mt-3 flex flex-wrap gap-1.5">
                        <Button type="button" variant={active ? "secondary" : "outline"} size="sm" disabled={active} onClick={() => { onOpen(item.id); onOpenChange(false); }}>
                          {active ? "Open now" : "Open"}
                        </Button>
                        <Button type="button" variant="ghost" size="icon" aria-label={`Duplicate ${item.label}`} title="Duplicate resume" onClick={() => { onDuplicate(item.id); onOpenChange(false); }}>
                          <Copy />
                        </Button>
                        <Button type="button" variant="ghost" size="icon" aria-label={`Rename ${item.label}`} title="Rename resume" onClick={() => { setRenamingId(item.id); setRenameDraft(item.label); }}>
                          <Pencil />
                        </Button>
                        <Button type="button" variant="ghost" size="icon" className="text-destructive hover:text-destructive" aria-label={`Delete ${item.label}`} title="Delete resume" onClick={() => setDeletingId(item.id)}>
                          <Trash2 />
                        </Button>
                      </div>
                    )}
            ...
            <div className="grid min-h-56 place-items-center rounded-lg border border-dashed bg-muted/30 p-6 text-center">
              <div>
                ...
                <Button type="button" variant="outline" size="sm" className="mt-4" onClick={() => setQuery("")}>Clear search</Button>
              </div>
            </div>
          )}
        </div>
