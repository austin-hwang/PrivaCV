...
async function svgToPng(svg: SVGSVGElement, fileName: string) {
  ...
  try {
    ...
    const blob = await new Promise<Blob>((resolve, reject) => {
      canvas.toBlob((result) => result ? resolve(result) : reject(new Error("The Sankey image could not be encoded.")), "image/png");
    });
...
export function JobPipelineSankey({
  ...
  const saveImage = async () => {
    ...
    setExporting(true);
    try {
      await svgToPng(svgRef.current, `privacv-job-search-sankey-${new Date().toISOString().slice(0, 10)}.png`);
      onExport({ ok: true, message: "Sankey image downloaded" });
    } catch (error) {
      onExport({ ok: false, message: error instanceof Error ? error.message : "The Sankey image could not be saved." });
    } finally {
      setExporting(false);
    }
  };
    ...
    return (
      <div className="flex min-h-[28rem] flex-col items-center justify-center px-6 text-center">
        ...
        <p className="mt-2 max-w-md text-sm leading-relaxed text-muted-foreground">
          Move an opportunity to Applied or a later stage. Saved and Preparing roles are intentionally left out of the job-search flow.
        </p>
      </div>
    );
  }
    ...
    <div className="p-4 sm:p-6">
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Flows are reconstructed from recorded application stages. Current filters are reflected.</p>
        </div>
        <Button type="button" variant="outline" onClick={saveImage} disabled={exporting}>
          {exporting ? <Loader2 className="animate-spin" /> : <Download />} {exporting ? "Rendering PNG" : "Save as PNG"}
        </Button>
      </div>
      ...
      <div className="overflow-x-auto rounded-xl border bg-white shadow-sm">
        ...
        >
          ...
          <desc id="job-sankey-description">{data.total} submitted applications flowing through interviews, offers, and outcomes.</desc>
          <rect width={VIEW_WIDTH} height={VIEW_HEIGHT} fill="#ffffff" />
          <text x="72" y="55" fill="#0f172a" fontFamily="ui-sans-serif, system-ui, sans-serif" fontSize="30" fontWeight="700">My job search</text>
          <text x="72" y="86" fill="#475569" fontFamily="ui-sans-serif, system-ui, sans-serif" fontSize="16">
            {data.total} submitted {data.total === 1 ? "application" : "applications"} · {date}
          </text>
          <text x="1128" y="55" textAnchor="end" fill="#2563eb" fontFamily="ui-sans-serif, system-ui, sans-serif" fontSize="17" fontWeight="700">PrivaCV</text>
          <text x="1128" y="82" textAnchor="end" fill="#64748b" fontFamily="ui-sans-serif, system-ui, sans-serif" fontSize="13">Private job search tracker</text>

          {layout.links.map((link) => (
            <path key={`${link.source}-${link.target}`} d={link.path} fill={link.color} fillOpacity="0.25" stroke={link.color} strokeOpacity="0.1">
              <title>{link.value} {link.value === 1 ? "application" : "applications"}: {link.source} to {link.target}</title>
            </path>
          ))}
          {layout.nodes.map((node) => {
            ...
            return (
              <g key={node.id}>
                ...
                >
                  {node.label}
                  <tspan dx="6" fill="#475569" fontWeight="500">{node.count} · {percentage}%</tspan>
                </text>
              </g>
            );
          })}
          <line x1="72" x2="1128" y1="620" y2="620" stroke="#e2e8f0" />
          <text x="72" y="650" fill="#64748b" fontFamily="ui-sans-serif, system-ui, sans-serif" fontSize="13">
            {data.excluded
      ...
      <ul className="sr-only">
        {data.nodes.map((node) => <li key={node.id}>{node.label}: {node.count}</li>)}
      </ul>
    </div>
  );
}
