...
Resume-export and application-creation metrics share a persistent random browser-profile token through `lib/visitor-metrics.ts`, stored in localStorage with Web Locks where available. Export blob3/application blob2 and each dataset's index1 contain the same ID. There is no separate workspace-visit tracker or dataset binding. Tokens persist across days and sessions until cleared; they identify a browser profile on this origin, not a physical device. GPC/DNT, unavailable storage, and desktop builds suppress collection. Old clients may send aggregate-only events; queries exclude their empty ID fields. Local-AI events remain identifier-free. The retired `/api/metrics/visitors` route returns 204 without recording anything, allowing older open clients to finish quietly. See `docs/visitor-analytics.md`.
