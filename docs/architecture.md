...
Hosted visitor metrics use a separate `privacv_visitors` dataset. `components/visitor-metrics.tsx` is mounted only on `/` and `/applications` with their respective workspace categories and records visible activity through `lib/visitor-metrics.ts`, sharing a random persistent browser-profile token in localStorage (with Web Locks where available). The Worker validates a bounded request containing only the current UTC day, UUID, and allowed workspace category, and uses workspace plus visitor ID as the Analytics Engine index. No content or request metadata is written. Tokens persist across UTC days, reloads, and sessions until cleared. They identify a browser profile on this origin, not a physical device. GPC/DNT, unavailable storage, and desktop builds suppress collection. Resume-export and application-creation events use the same ID, in export blob3/application blob2 and each dataset's index1. Old clients may send aggregate-only events; queries exclude their empty ID fields. Local-AI events remain identifier-free. See `docs/visitor-analytics.md` for queries and limits.
...
Resume edits are journaled synchronously in localStorage until their IndexedDB transaction commits, allowing interrupted saves to recover on reload. Writes compare changed records against their original values within a transaction, preserving unrelated changes from other tabs. Conflicting interrupted edits recover as separate drafts. Commit notifications refresh other tabs; checkpoint writes add or remove only the affected records. Application forms similarly submit changed fields with their original values, so saving notes cannot revert a status changed elsewhere.
...
The editor and tracker do not upload resume or application data. Hosted network activity is limited to ordinary page/assets requests, an explicit optional model download, the explicit WebRTC handoff's encrypted short-lived signaling plus STUN/TURN connectivity, external links a user opens, and the documented product-metric endpoints. The signaling room processes a random room ID, encrypted offer and answer, expiry time, and ordinary request metadata. STUN processes connection metadata such as IP addresses and ports. When direct connectivity fails, TURN additionally processes temporary credentials and relays encrypted packet bytes. Neither service can read the resume or application payload. Metrics accept only allowlisted event names/formats and, for workspace visits, exports, and application creation, a validated random browser-profile UUID. Workspace visits also include UTC day and a fixed workspace category. They must never accept arbitrary resume, application, prompt, generated-text, account, or hardware/device fields.

In Electron, page, asset, API, and export-metric requests resolve against the bundled loopback server. The packaged metric handlers are no-ops, so desktop usage events do not leave the device. Optional model preparation still downloads model files from the allowlisted hosts; device handoff explicitly contacts `privacv.app` for encrypted signaling and temporary TURN credentials, then Cloudflare's STUN/TURN service for connectivity; and explicit external links still require network access. Core editing, local storage, imports, and exports work without internet access.
